...
# title_episode_df = title_episode_df.withColumn("seasonNumber", when(col("seasonNumber") == "\\N", -1).otherwise(col("seasonNumber").cast(IntegerType())))
# title_episode_df = title_episode_df.withColumn("episodeNumber", when(col("episodeNumber") == "\\N", -1).otherwise(col("episodeNumber").cast(IntegerType())))

# Dataset analysis
show_data_summary(title_ratings_df, "Title Ratings DataFrame", extended_stats=True)
show_data_summary(title_episode_df, "Title Episode DataFrame", extended_stats=True)
show_data_summary(title_akas_df, "Title Akas DataFrame", extended_stats=True)
show_data_summary(title_crew_df, "Title Crew DataFrame", extended_stats=True)
show_data_summary(title_basics_df, "Title Basics DataFrame", extended_stats=True)
show_data_summary(name_basics_df, "Name Basics DataFrame", extended_stats=True)
show_data_summary(title_principals_df, "Title Principals DataFrame", extended_stats=True)
